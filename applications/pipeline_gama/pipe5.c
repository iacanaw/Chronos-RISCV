...
static char start_print[] = "Starting PIPELINE GAMA - pipe 5 \n";
...
int main(){
    ...
    /* main loop */
    for (/* i = 0 */; i < (VAR_ITERATION/650)+3; i++) {
        
        checkMigration();

        sys_Receive(&mensagem, pipe4);
        sum = mensagem.msg[0];

        for(inner = 0; inner < INNER_LOOP; inner = inner + 6){
            sum += 6;
        }
